...
#define TTGOVGARESOLUTION VGA_640x350_70Hz
//#define TTGOVGARESOLUTION VGA_512x448_60Hz
#endif
...
void vgabegin() {
  //VGAController.setAudioEnabled(false);
  ...
  VGAController.setResolution(TTGOVGARESOLUTION);
	...
	Terminal.setBackgroundColor(invertColor(vga_txt_background));
	Terminal.setForegroundColor(invertColor(vga_txt_pen));
 #ifdef TTGOVGAFONT
...
void rgbcolor(uint8_t r, uint8_t g, uint8_t b) {
	...
	if (r>191 || g>191 || b>191) vga=8; else vga=0;
	vga=vga+r/128+g/128*2+b/128*4;
vga_graph_pen = invert(Color(vga));
vga_txt_background = invert(Color(vga));
...
void plot(int x, int y) { 
	vgascale(&x, &y);
	cv.setPenColor(invertColor(vga_graph_pen));
	cv.setPixel(x,y);
	cv.setPenColor(invertColor(vga_txt_pen));
}
...
void frect(int x0, int y0, int x1, int y1) {  
	vgascale(&x0, &y0);
	vgascale(&x1, &y1);
	cv.setBrushColor(invertColor(vga_graph_pen));
	cv.fillRectangle(x0, y0, x1, y1);
	cv.setBrushColor(invertColor(vga_txt_background));
}
